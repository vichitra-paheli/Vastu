/**
 * baseSeed.ts — Vastu framework base seed
 *
 * Exports runBaseSeed() which seeds all framework-level data:
 * organization, tenant, system users, system roles, permissions,
 * DB connections, API keys, and sample audit events.
 *
 * This is called by:
 *   1. packages/shared/src/prisma/seed.ts (framework dev seed)
 *   2. apps/<name>/prisma/seed.ts (app seeds — called first, then domain seed)
 *
 * The seed is idempotent: all upserts use deterministic UUIDs.
...
async function seedOrganization(prisma: PrismaClient): Promise<void> {
  console.log('Seeding organizations...');
  await prisma.organization.upsert({
    where: { id: BASE_SEED_IDS.org },
    update: {},
    create: {
      id: BASE_SEED_IDS.org,
      name: 'Acme Corp',
    },
  });
  console.log('  done');
...
async function seedTenant(prisma: PrismaClient): Promise<void> {
  ...
  await prisma.tenant.upsert({
    where: { id: BASE_SEED_IDS.tenant },
    update: {},
    create: {
      id: BASE_SEED_IDS.tenant,
      name: 'Default',
      subdomain: 'default',
      status: TenantStatus.ACTIVE,
      organizationId: BASE_SEED_IDS.org,
    },
  });
  console.log('  done');
}
...
async function seedUsers(prisma: PrismaClient): Promise<void> {
  ...
  const users = [
    {
      id: BASE_SEED_IDS.users.admin,
      email: 'admin@vastu.dev',
      name: 'Admin User',
      emailVerified: true,
    },
    {
      id: BASE_SEED_IDS.users.editor,
      email: 'editor@vastu.dev',
      name: 'Editor User',
      emailVerified: true,
    },
    {
      id: BASE_SEED_IDS.users.viewer,
...
async function seedUserRoles(prisma: PrismaClient): Promise<void> {
  console.log('Seeding user-role assignments...');

  const assignments = [
    {
      id: BASE_SEED_IDS.userRoles.adminAdmin,
      userId: BASE_SEED_IDS.users.admin,
      roleId: BASE_SEED_IDS.roles.admin,
      tenantId: BASE_SEED_IDS.tenant,
    },
    {
      id: BASE_SEED_IDS.userRoles.editorEditor,
      userId: BASE_SEED_IDS.users.editor,
      roleId: BASE_SEED_IDS.roles.editor,
      tenantId: BASE_SEED_IDS.tenant,
    },
    {
      id: BASE_SEED_IDS.userRoles.viewerViewer,
      userId: BASE_SEED_IDS.users.viewer,
      roleId: BASE_SEED_IDS.roles.viewer,
      tenantId: BASE_SEED_IDS.tenant,
    },
  ];
...
async function seedPermissions(prisma: PrismaClient): Promise<void> {
  ...
  const specs: PermissionSpec[] = [
    ...
    {
      roleId: BASE_SEED_IDS.roles.builder,
      roleKey: 'builder',
      actions: ['read', 'export'],
    },
    {
...
async function seedDbConnections(prisma: PrismaClient): Promise<void> {
  console.log('Seeding DB connections...');

  const connections = [
    {
      id: BASE_SEED_IDS.dbConnections.production,
    ...
    {
      id: BASE_SEED_IDS.dbConnections.analytics,
...
async function seedApiKeys(prisma: PrismaClient): Promise<void> {
  console.log('Seeding API keys...');

  const keys = [
    {
      id: BASE_SEED_IDS.apiKeys.cicd,
    ...
    {
      id: BASE_SEED_IDS.apiKeys.readOnly,
...
async function seedAuditEvents(prisma: PrismaClient): Promise<void> {
  ...
  const events: AuditEventSeed[] = [
    {
      ...
      action: 'user.login',
      resourceType: 'User',
      resourceId: BASE_SEED_IDS.users.admin,
    ...
    },
    {
      ...
      action: 'user.create',
      resourceType: 'User',
      resourceId: BASE_SEED_IDS.users.editor,
    ...
    },
    {
      ...
      action: 'user.create',
      resourceType: 'User',
      resourceId: BASE_SEED_IDS.users.viewer,
    ...
    },
    {
      ...
      action: 'role.update',
      resourceType: 'Role',
      resourceId: BASE_SEED_IDS.roles.editor,
    ...
    },
    {
      ...
      resourceType: 'ApiKey',
      resourceId: BASE_SEED_IDS.apiKeys.cicd,
    ...
    },
    {
      ...
      action: 'api_key.create',
      resourceType: 'ApiKey',
      resourceId: BASE_SEED_IDS.apiKeys.readOnly,
    ...
    },
    {
      ...
      resourceId: BASE_SEED_IDS.dbConnections.production,
    ...
    },
    {
      ...
      action: 'db_connection.test',
      resourceType: 'DbConnection',
      resourceId: BASE_SEED_IDS.dbConnections.analytics,
    ...
    },
    {
      ...
      resourceType: 'Tenant',
      resourceId: BASE_SEED_IDS.tenant,
    ...
    },
    {
      ...
      resourceId: BASE_SEED_IDS.roles.builder,
    ...
    },
    {
      ...
      action: 'role.update',
      resourceType: 'Role',
      resourceId: BASE_SEED_IDS.roles.viewer,
...
/**
 * runBaseSeed — Seeds all Vastu framework base data.
 *
 * Called by the framework dev seed and by app seed files as their first step.
 * Pass your app's PrismaClient instance; this function does not create its own.
 *
 * Example app usage:
 ...
 */
export async function runBaseSeed(prisma: PrismaClient): Promise<void> {
  console.log('Running Vastu base seed...');
