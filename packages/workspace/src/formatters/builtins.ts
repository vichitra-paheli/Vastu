...
const DATE_FORMATTER: FormatterDefinition = {
  ...
  sort({ aValue, bValue }) {
    // Sort chronologically: parse to timestamp, nulls last
...
const AVATAR_FORMATTER: FormatterDefinition = {
  ...
  render({ value, row }) {
    const str = toStr(value);
    // Derive initials: if the value looks like a name, use first letters of first 2 words
    ...
    // Try to resolve an image URL from a companion field (avatarUrl, imageUrl, avatar)
    const imageUrl =
...
const LINK_FORMATTER: FormatterDefinition = {
  ...
  render({ value, row }) {
    ...
    if (!href) return null;
    // Prefer a companion label field (linkLabel, label, name), fall back to the URL
 ...
 *
 * The module-level call at the bottom runs this once at startup.
 */
export function registerBuiltinFormatters(): void {
  for (const def of BUILTIN_FORMATTERS) {
    // Use hasFormatter to skip already-registered formatters rather than throwing
    // This allows safe re-invocation after clearFormatterRegistry() in tests.
